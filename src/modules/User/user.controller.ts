...
export const fetchAllProfiles = async (req: Request, res: Response) => {
  try {
    ...
    const parsedMinGenderProbability  = min_gender_probability  !== undefined ? Number(min_gender_probability)  : undefined;
    const parsedMinCountryProbability = min_country_probability !== undefined ? Number(min_country_probability) : undefined;
    ...
    if (parsedMinGenderProbability  !== undefined && isNaN(parsedMinGenderProbability))  throw customBadRequestError("min_gender_probability must be a number");
    if (parsedMinCountryProbability !== undefined && isNaN(parsedMinCountryProbability)) throw customBadRequestError("min_country_probability must be a number");
